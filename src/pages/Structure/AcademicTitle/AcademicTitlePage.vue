          ...
          <n-drawer-content>
            ...
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Unvon nomi" path="name">
...
const get_tableList = (load =true) => {
  loading.value = load;
  let param = params.value
  $ApiService.academicTitle.academicTitle_index(param).then((res) => {
    total_item.value = res.data.academic_titles.total;
    table_list.value = res.data.academic_titles.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.academicTitle.academicTitle_edit({title_id:item.id}).then((res)=>{
    item_details.value.name = res.data.language.name;
  }).finally(()=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.academicTitle.academicTitle_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.academicTitle.academicTitle_update({data:item_details.value, title_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.academicTitle.academicTitle_delete({title_id:event.data.id}).then((res)=>{
