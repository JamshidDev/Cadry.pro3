          ...
          <n-drawer-content>
            ...
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Qarindoshligi" path="name">
...
const get_tableList = () => {
  ...
  $ApiService.relatives.relative_index(param).then((res) => {
    total_item.value = res.data.relatives.total;
    table_list.value = res.data.relatives.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.relatives.relative_edit({relative_id:item.id}).then((res)=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.relatives.relative_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.relatives.relative_update({data:item_details.value, relative_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.relatives.relative_delete({relative_id:event.data.id}).then((res)=>{
