          ...
          <n-drawer-content>
            <template #header>
              <span class="primary-text"> {{
                  dialog_type ? "Yangi Millat qo'shish" : "Ma'lumotlarni tahrirlash"
                }}</span>
            </template>
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Millat nomi" path="name">
...
const get_tableList = () => {
  ...
  $ApiService.nationality.nationality_index(param).then((res) => {
    total_item.value = res.data.nationalities.total;
    table_list.value = res.data.nationalities.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.nationality.nationality_edit({nationality_id:item.id}).then((res)=>{
    item_details.value.name = res.data.nationality.name;
  }).finally(()=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.nationality.nationality_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.nationality.nationality_update({data:item_details.value, nationality_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.nationality.nationality_delete({nationality_id:event.data.id}).then((res)=>{
