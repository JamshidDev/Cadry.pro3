          ...
          <n-drawer-content>
            <template #header>
              <span class="primary-text"> {{
                  dialog_type ? "Yangi Daraja qo'shish" : "Ma'lumotlarni tahrirlash"
                }}</span>
            </template>
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Daraja nomi" path="name">
...
const get_tableList = () => {
  ...
  $ApiService.academicDegree.academicDegree_index(param).then((res) => {
    total_item.value = res.data.academic_degrees.total;
    table_list.value = res.data.academic_degrees.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.academicDegree.academicDegree_edit({degree_id:item.id}).then((res)=>{
    item_details.value.name = res.data.academic_degree.name;
  }).finally(()=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.academicDegree.academicDegree_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.academicDegree.academicDegree_update({data:item_details.value, degree_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.academicDegree.academicDegree_delete({degree_id:event.data.id}).then((res)=>{
