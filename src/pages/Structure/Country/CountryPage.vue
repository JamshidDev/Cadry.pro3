          ...
          <n-drawer-content>
            <template #header>
              <span class="primary-text"> {{
                  dialog_type ? "Yangi Davlat qo'shish" : "Ma'lumotlarni tahrirlash"
                }}</span>
            </template>
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Davlat nomi" path="name">
                  <n-input v-model:value="item_details.name" type="text" placeholder="Kiriting"/>
                </n-form-item>
                <n-form-item label="Davlat nomining qisqa kodi" path="short_code">
...
const get_tableList = () => {
  ...
  $ApiService.countries.country_index(param).then((res) => {
    total_item.value = res.data.countries.total;
    table_list.value = res.data.countries.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.countries.country_edit({country_id:item.id}).then((res)=>{
    item_details.value.name = res.data.country.name;
    item_details.value.short_code = res.data.country.short_code;
  }).finally(()=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.countries.country_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.countries.country_update({data:item_details.value, country_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.countries.country_delete({country_id:event.data.id}).then((res)=>{
