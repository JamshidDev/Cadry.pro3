          ...
          <n-drawer-content>
            ...
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Til" path="name">
...
const get_tableList = () => {
  ...
  $ApiService.languages.language_index(param).then((res) => {
    total_item.value = res.data.languages.total;
    table_list.value = res.data.languages.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.languages.language_edit({language_id:item.id}).then((res)=>{
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.languages.language_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.languages.language_update({data:item_details.value, language_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.languages.language_delete({language_id:event.data.id}).then((res)=>{
