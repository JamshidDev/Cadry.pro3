          ...
          <n-drawer-content>
            ...
            <n-spin :show="drawer_loading" >
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Davlat nomi" path="country_id">
                  <n-select
                      v-model:value="item_details.country_id"
                      filterable
                      placeholder="Tanlang"
                ...
                <n-form-item label="Viloyat nomi" path="name">
                  <n-input v-model:value="item_details.name" type="text" placeholder="Kiriting"/>
                </n-form-item>
                <n-form-item label="Viloyat nomining qisqa kodi" path="short_code">
                  <n-input v-model:value="item_details.short_code" type="text" placeholder="Kiriting"/>
                </n-form-item>
              </n-form>
            </n-spin>
...
const get_tableList = () => {
  ...
  $ApiService.regions.region_index(param).then((res) => {
    total_item.value = res.data.regions.total;
    table_list.value = res.data.regions.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const get_countries = () => {
  if (country_list.value.length == 0) {
    drawer_loading.value = true;
    $ApiService.regions.region_create().then((res) => {
      country_list.value = res.data.countries;
    }).finally(() => {
...
const edit_item = (item) => {
  ...
  $ApiService.regions.region_edit({region_id:item.id}).then((res)=>{
    country_list.value = res.data.countries;
    item_details.value.country_id = res.data.region.country.id;
    item_details.value.name = res.data.region.name;
    item_details.value.short_code = res.data.region.short_code;
  }).finally(()=>{
    drawer_loading.value = false;
  })
...
const addAndEdit = () => {
  formRef.value.validate((error)=>{
    if(!error && dialog_type.value){
      $ApiService.regions.region_store({data:item_details.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }else if(!error && !dialog_type.value){
      $ApiService.regions.region_update({data:item_details.value, region_id:selected_item_id.value}).then((res)=>{
        dialog.value = false;
        get_tableList()
      })
    }
...
const delete_item = (event)=>{
  $ApiService.regions.region_delete({region_id:event.data.id}).then((res)=>{
