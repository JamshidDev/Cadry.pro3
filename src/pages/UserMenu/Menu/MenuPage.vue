          ...
          <div class="w-full overflow-x-auto" v-if="table_list.length>0 && !loading">
            <n-table :bordered="false" :single-line="false" size="small" striped>
              ...
              <tr>
                ...
                <th class="text-dark-color" style="min-width: 80px; width: 200px">Server Route</th>
                <th class="text-dark-color" style="min-width: 80px; width: 200px">Client Route</th>
                <th class="text-dark-color" style="min-width: 80px; width: 200px">Prefix</th>
          ...
          <n-drawer-content>
            <template #header>
              <span class="primary-text"> {{
                  dialog_type ? "Yangi Menu qo'shish" : "Ma'lumotlarni tahrirlash"
                }}</span>
            </template>
            <n-spin :show="drawer_loading">
              <n-form ref="formRef" :model="item_details" :rules="rules">
                <n-form-item label="Menu nomi" path="name">
                  <n-input v-model:value="item_details.name" type="text" placeholder="Kiriting"/>
                </n-form-item>
                <n-form-item label="Server route" path="route">
                  <n-input v-model:value="item_details.route" type="text" placeholder="Kiriting"/>
                </n-form-item>
                <n-form-item label="Client route" path="front_route">
                  <n-input v-model:value="item_details.front_route" type="text" placeholder="Kiriting"/>
                </n-form-item>
                <n-form-item label="Prefix" path="prefix">
                  <n-input v-model:value="item_details.prefix" type="text" placeholder="Kiriting"/>
                </n-form-item>
              </n-form>
            </n-spin>
...
const get_tableList = () => {
  ...
  $ApiService.menu.menu_index(param).then((res) => {
    total_item.value = res.data.menus.total;
    table_list.value = res.data.menus.data;
  }).finally(() => {
    loading.value = false;
  })
}
...
const edit_item = (item) => {
  ...
  $ApiService.menu.menu_edit({menu_id: item.id}).then((res) => {
    item_details.value.name = res.data.menu.name;
    item_details.value.route = res.data.menu.route;
    item_details.value.front_route = res.data.menu.front_route;
    item_details.value.prefix = res.data.menu.prefix;
  }).finally(() => {
...
const addAndEdit = () => {
  formRef.value.validate((error) => {
    if (!error && dialog_type.value) {
      $ApiService.menu.menu_store({data: item_details.value}).then((res) => {
        dialog.value = false;
        get_tableList()
      })
    } else if (!error && !dialog_type.value) {
      $ApiService.menu.menu_update({
        data: item_details.value,
        menu_id: selected_item_id.value
      }).then((res) => {
        dialog.value = false;
        get_tableList()
      })
...
const delete_item = (event) => {
  $ApiService.menu.menu_delete({menu_id: event.data.id}).then((res) => {
